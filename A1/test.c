...
int main(int argc, char *argv[]) {
    ...
    outFile = open(outFilePath, O_APPEND|O_WRONLY|O_CREAT, S_IRUSR | S_IWUSR);
    int totalChars = lseek(inFile, 0, SEEK_END);
    int lastOffset = totalChars;
    int currentOffset = totalChars - BATCH_SIZE;
    ...
    while(!flag){
        ...
        sprintf(outputtext, "\rPercentage Done: %.2f %%", perc);
        write(1, outputtext, 25);
